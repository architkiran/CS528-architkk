...
echo 'export GCE_METADATA_MTLS_MODE=none' >> /etc/environment

BUCKET_NAME=$(curl -sf \
    "http://metadata.google.internal/computeMetadata/v1/instance/attributes/bucket-name" \
    -H "Metadata-Flavor: Google")
FORBIDDEN_URL=$(curl -sf \
    "http://metadata.google.internal/computeMetadata/v1/instance/attributes/forbidden-service-url" \
    -H "Metadata-Flavor: Google")
...
def _get_zone():
    try:
        ...
        with urllib.request.urlopen(req, timeout=2) as resp:
            return resp.read().decode().strip().split("/")[-1]
    except Exception:
        return "unknown"
...
class GCSHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not filename:
            ...
            self.send_header("X-Zone", INSTANCE_ZONE)
            self.end_headers()
...
if __name__ == "__main__":
    print(f"Starting server on port {PORT} (zone: {INSTANCE_ZONE})...", flush=True)
...
[Unit]
Description=GCS Web Server (HW8)
